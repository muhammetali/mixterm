...
import 'package:xterm/src/core/cell.dart'; // For CellData, CellContent (per-cell selection)
...
class RenderTerminal extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  void _paintSelection(
    ...
    final themeBg = _painter.theme.background;

    for (final segment in selection.toSegments()) {
      if (segment.line >= lines.length) {
        break;
      }
      ...
      // Per-cell invert: redraw each selected cell with inverted colors
      final line = lines[segment.line];
      final start = segment.start ?? 0;
      final end = segment.end ?? _terminal.viewWidth;
      final lineY = segment.line * cellHeight + _lineOffset;
      ...
      for (var col = start; col < end; col++) {
        ...
        final cellOffset = Offset(col * cellWidth, lineY);

        // Paint this cell with inverted colors
        _painter.paintCellInverted(canvas, cellOffset, cellData, themeBg);

        // Handle double-width characters
        final charWidth = cellData.content >> CellContent.widthShift;
        if (charWidth == 2) {
          col++; // Skip next column (it's part of this wide char)
        }
      }
    }
  }
