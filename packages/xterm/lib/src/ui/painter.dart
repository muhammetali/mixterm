...
class TerminalPainter {
  ...
  // ============================================================
  // Per-cell selection invert support (MixTerm fork)
  // ============================================================

  /// Inverts a color by subtracting each RGB component from 255.
  /// This creates a true terminal-style inverted selection effect.
  @pragma('vm:prefer-inline')
  static Color invertColor(Color c) {
    // Using new Color API to avoid deprecation warnings
  ...
  /// Paints a cell with inverted colors for selection highlighting.
  /// This method redraws both background and foreground with colors swapped/inverted.
  void paintCellInverted(
    Canvas canvas,
    Offset offset,
    CellData cellData,
    Color themeBg,
  ) {
    // Get the effective foreground and background colors
    final cellFlags = cellData.flags;
    final isInverse = cellFlags & CellFlags.inverse != 0;

    // Resolve actual fg/bg considering the cell's inverse flag
    Color effectiveFg;
    Color effectiveBg;

    if (isInverse) {
      effectiveFg = resolveBackgroundColor(cellData.background);
      effectiveBg = resolveForegroundColor(cellData.foreground);
    } else {
      effectiveFg = resolveForegroundColor(cellData.foreground);
      // For background, check if it's transparent/normal
    ...
    final selectionFg = effectiveBg;

    // Paint inverted background (always paint for selection, even if originally transparent)
    final doubleWidth = cellData.content >> CellContent.widthShift == 2;
    final widthScale = doubleWidth ? 2 : 1;
    final bgSize = Size(_cellSize.width * widthScale + 1, _cellSize.height);
    final bgPaint = Paint()..color = selectionBg;
    canvas.drawRect(offset & bgSize, bgPaint);

    // Paint inverted foreground (text)
    final charCode = cellData.content & CellContent.codepointMask;
    if (charCode == 0) return;

    // Apply faint effect if present
    var fgColor = selectionFg;
    ...
    // Handle space underline issue (same as paintCellForeground)
    var char = String.fromCharCode(charCode);
    if (cellFlags & CellFlags.underline != 0 && charCode == 0x20) {
      char = String.fromCharCode(0xA0);
    }

    // Build and draw the paragraph
    // Note: We don't use cache here because inverted cells are temporary (selection)
    final builder = ParagraphBuilder(style.getParagraphStyle());
    builder.pushStyle(style.getTextStyle(textScaler: _textScaler));
    builder.addText(char);

    final paragraph = builder.build();
    paragraph.layout(ParagraphConstraints(width: _cellSize.width * 2));
    canvas.drawParagraph(paragraph, offset);
    paragraph.dispose();
  }
}
